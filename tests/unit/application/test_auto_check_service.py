...
class TestReanalysisDoesNotFabricateUntimedCheckpoints:
    """重分析/刷节奏点不得把已打轴字符的 check_count 撑到超过已有时间戳。

    回归：KRL→自动注音后，汉字的读音被拆成多个 mora，节奏点随之增加，但
    文件/手动只给了 1 个时间戳 → is_fully_timed 判为“未打轴”，导出弹
    “只有 0/N 行完成打轴”；而预览会把无 ts 的节奏点并入前一段走字，表现为
    “预览正常、导出报未打轴”的假警。这里锁死“节奏点收口到已有时间戳粒度”。
    """

    _FLAGS = {"check_line_end": False, "check_space_as_line_end": False}

    def _refresh(self, timestamps, parts):
        ...
        ch = Character(
            ...
            ruby=Ruby(parts=[RubyPart(text=t) for t in parts]),
            singer_id="s1",
        )
        sentence = Sentence(singer_id="s1", characters=[ch])
        AutoCheckService(
            object(), auto_check_flags=self._FLAGS
        ).update_checkpoints_from_rubies(sentence)
        return sentence.characters[0]

    def test_one_timestamp_two_mora_merges_back_to_one_checkpoint(self):
        ch = self._refresh([14740], ["わ", "す"])
        assert ch.check_count == 1
        assert ch.timestamps == [14740]
        assert [p.text for p in ch.ruby.parts] == ["わす"]
        assert ch.is_fully_timed

    def test_two_timestamps_two_mora_keeps_two_checkpoints(self):
        ch = self._refresh([14740, 14940], ["わ", "す"])
        assert ch.check_count == 2
        assert ch.timestamps == [14740, 14940]
        assert [p.text for p in ch.ruby.parts] == ["わ", "す"]
        assert ch.is_fully_timed

    def test_three_timestamps_three_mora_keeps_three_checkpoints(self):
        ch = self._refresh([1000, 1200, 1400], ["ひ", "か", "り"])
