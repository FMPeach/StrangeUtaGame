...
class TestLRCParser:
    ...
    def test_word_by_word_trailing_full_width_space_keeps_release_timestamp(self):
        parser = LRCParser()
        content = "[00:00.000]春[00:01.000]日[00:02.000]\u3000"

        result = parser.parse(content)

        assert len(result) == 1
        assert result[0].text == "春日"
        assert result[0].timetags == [(0, 0), (1, 1000)]
        assert result[0].line_end_ts == 2000
    ...
    def test_enhanced_lrc_trailing_full_width_space_keeps_release_timestamp(self):
        parser = LRCParser()
        content = "[00:00.000]<00:00.000>春<00:01.000>日<00:02.000>\u3000"
...
class TestNicokaraTagsRoundTrip:
    ...
    def test_detected_nicokara_syncs_offset_and_custom_tags(self):
        ...
        setting_iface = _SettingInterface()
        content = (
        ...
        _, is_nicokara, _, _ = parse_lyric_content(
            content,
            "singer_1",
