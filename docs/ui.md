...
- **职责**：
    ...
    - **打轴设定**：默认打轴偏移 0ms；Karaoke 渲染/导出偏移 -230ms；微调时间戳步长 10ms；快捷键配置（`add_checkpoint` F5、`remove_checkpoint` F6、`toggle_line_end` .、`toggle_word_join` F3 等）。支持长按绑定（按键持续 300ms 以上触发不同功能）。
